...
import os
# 1. 自动获取项目根目录（MOL-Mamba），适配任意运行位置
# __file__ 是 mol_bpe.py 的绝对路径，向上3级定位根目录：
# datasets/fragment/mol_bpe.py -> datasets/fragment/ -> datasets/ -> MOL-Mamba/
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
# 2. 定义日志目录（绝对路径，指向预期的 datasets/ 目录，无重复嵌套）
log_dir = os.path.join(PROJECT_ROOT, "datasets")
# 3. 确保日志目录存在（关键！避免因目录不存在导致创建日志文件失败）
...
# pycharm路径
#logger = setup_logger('Data Prepare', './datasets', filename="data_log.txt")

#Linxu路径
#log_dir = './datasets/fragment'  # 直接用 fragment 目录  Linux可以 pycharm不行
log_dir = './datasets'
#logger = setup_logger('Data Prepare', log_dir, filename='data_log.txt')
logger = setup_logger('Data Prepare', log_dir, filename='data_log.txt')
...
class MolInSubgraph:
    ...
    def __init__(self, mol, kekulize=False):
        ...
        for atom in mol.GetAtoms():
            ...
            self.subgraphs[idx] = {idx: symbol}  #idx是当前原子在所属的Mol对象中的索引，一个原子有一个索引 而不是一个元素一个 eg水中两个氢原子的索引是两个不同值
    ...
    def get_nei_subgraphs(self):
        nei_subgraphs, merge_pids = [], []  #nei可能是neighbor的缩写
        #key对应子图
        for key in self.subgraphs:
            subgraph = self.subgraphs[key]
            local_nei_pid = []
            for aid in subgraph:
                ...
                for nei in atom.GetNeighbors():
                    nei_idx = nei.GetIdx()
                    if nei_idx in subgraph or nei_idx > aid:  # 避免重复计算边 邻居原子索引若大于当前原子索引就不计算了
                        continue
                    local_nei_pid.append(self.inversed_index[nei_idx])  #inverse将外部索引反推为内部索引
            local_nei_pid = set(local_nei_pid)   #局部邻居父节点
        ...
        return nei_subgraphs, merge_pids  #nei_subgraphs (相邻子图列表) merge_pids(合并子图对列表)
    ...
    def get_nei_smis(self):
        if self.dirty:
            ...
            for i, subgraph in enumerate(nei_subgraphs):
                submol = get_submol(self.mol, list(subgraph.keys()), kekulize=self.kekulize) #get_submol是从原始分子中提取新的子分子对象
                smi = mol2smi(submol)
                nei_smis.append(smi)
                self.smi2pids.setdefault(smi, [])  #如果key存在就返回对应的值 如果不存在就把他加入字典中并设置默认值
...
def graph_bpe(fname, vocab_len, vocab_path, cpus, kekulize):
    # load molecules
    logger.info(f'Loading mols from {fname} ...')
    ...
    logger.info(f'Added {len(selected_smis)} atoms, {add_len} principal subgraphs to extract')
    ...
    while len(selected_smis) < vocab_len:
        ...
        pbar.update(1)
    pbar.close()
    logger.info('sorting vocab by atom num')
...
def parse():
    ...
    parser.add_argument('--data', type=str,
                        default=r'', help='Path to molecule corpus')
    parser.add_argument('--vocab_size', type=int,
                        default=800, help='Length of vocab')
    parser.add_argument('--output', type=str,
                        default='./chem_vocab.txt', help='Path to save vocab')
    #################################

    parser.add_argument('--workers', type=int, default=0, help='Number of cpus to use')
