...
class GINE(BasicGNN):
    supports_edge_weight = False  #不处理边的权重
    supports_edge_attr = True

    def init_conv(self, in_channels: int, out_channels: int, edge_dim: int, **kwargs) -> MessagePassing:
        mlp = MLP(
            [in_channels, out_channels, out_channels],  #两个隐藏层
            act=self.act,
            act_first=self.act_first,
            norm=self.norm,
            norm_kwargs=self.norm_kwargs,
        )
        return GINEConv(mlp, train_eps=True, edge_dim=edge_dim, **kwargs)  #**kwargs用于打包参数
...
class GINConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        edge_index = add_self_loops(edge_index, num_nodes=x.size(0))  #给自己添加一个环

        self_loop_attr = torch.zeros(x.size(0), 2)
        self_loop_attr[:, 0] = 4  # bond type for self-loop edge
        self_loop_attr = self_loop_attr.to(edge_attr.device).to(edge_attr.dtype)
        edge_attr = torch.cat((edge_attr, self_loop_attr), dim=0)   #拼接原始边属性和自边环属性

        # 边属性嵌入：将离散的边属性转换为连续向量
        edge_embeddings = self.edge_embedding1(edge_attr[:, 0]) + \
                          self.edge_embedding2(edge_attr[:, 1])      #键类型 方向属性
...
class GIN(torch.nn.Module):
    ...
    # def forward(self, x, edge_index, edge_attr):  输入处理
...
class GNN(nn.Module):
    def __init__(self, num_layer, emb_dim, JK="last", drop_ratio=0., gnn_type="gin"):
        ...
        self.drop_ratio = drop_ratio
        self.num_layer = num_layer
        self.JK = JK  #Jump Knowledge跨层特征融合策略
