...
def mol_to_graph_data_obj_pos(mol, smiles):
    """
    ...
    try:
        ...
        #用RDKit的嵌入算法生成初始3D坐标，之后进行力场优化
        AllChem.EmbedMolecule(mol)
        AllChem.UFFOptimizeMolecule(mol)
        mol = Chem.RemoveHs(mol)#移除显式氢
        conformer = mol.GetConformer()

    except (RuntimeError, ValueError):

        # 捕获异常后，确保重新计算二维坐标前更新分子属性缓存
        for atom in mol.GetAtoms():
            atom.UpdatePropertyCache()
    ...
    data = Data(x=x, pos=pos, edge_index=edge_index, edge_attr=edge_attr, smiles=smiles)
