...
def compute_spearmanr_metric(y_true, y_pred):
...
if __name__ == '__main__':
    ...
    for epoch in range(cfg.epoch):
        ...
        train_res = compute_spearmanr_metric(y_true, y_pred)

        print_loss = cum_loss / len(trainloader)
        writer.add_scalar('Training Loss', print_loss, epoch)
        writer.add_scalar('Train Spearman', train_res, epoch)
        ...
        y_true = torch.cat(y_true, dim=0).numpy()
        y_pred = torch.cat(y_pred, dim=0).numpy()
        val_res = compute_spearmanr_metric(y_true, y_pred)
        writer.add_scalar('Valid Spearman', val_res, epoch)
        ...
        y_true = torch.cat(y_true, dim=0).numpy()
        y_pred = torch.cat(y_pred, dim=0).numpy()
        test_res = compute_spearmanr_metric(y_true, y_pred)
        writer.add_scalar('Test Spearman', test_res, epoch)
        ...
        logger.info(
            ...
            f"Train Spearman: {train_res:.4f}, Val Spearman: {val_res:.4f}, Test Spearman: {test_res:.4f}"
        )
