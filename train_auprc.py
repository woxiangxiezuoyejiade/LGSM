...
if __name__ == '__main__':
    ...
    for epoch in range(cfg.epoch):
        ...
        y_true = torch.cat(y_true, dim=0).numpy()
        y_pred = torch.cat(y_pred, dim=0).numpy()
        val_res = prc_auc(y_true, y_pred)
        writer.add_scalar('Valid AUPRC', val_res, epoch)
        ...
        y_true = torch.cat(y_true, dim=0).numpy()
        y_pred = torch.cat(y_pred, dim=0).numpy()
        test_res = prc_auc(y_true, y_pred)
        writer.add_scalar('Test AUPRC', test_res, epoch)
        ...
        logger.info(
            ...
            f"Train AUPRC: {train_res:.4f}, Val AUPRC: {val_res:.4f}, Test AUPRC: {test_res:.4f}"
        )
